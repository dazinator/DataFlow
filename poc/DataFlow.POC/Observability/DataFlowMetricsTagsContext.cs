...
public class DataFlowMetricsTagsContext : IMetricsTagsContext
{
    ...
    /// <summary>
    /// Flow-level tags that are applicable for metrics that aggregate across all instances / executions of this flow.
    /// </summary>
    public KeyValuePair<string, object?>[] FlowWideTags { get; }

    /// <summary>
    /// Flow-instance level tags that are applicable for this specific execution / invocation of the flow (by invocationId).
    /// </summary>
    public KeyValuePair<string, object?>[] FlowInstanceTags { get; }

    /// <summary>
    /// Flow-level tags that are applicable for metrics that aggregate across all instances / executions of a flow name (i.e. not labelled to specific instance / invocation)
    /// </summary>
    public KeyValuePair<string, object?>[]? FlowLevelCompletionTags { get; private set; }

    /// <summary>
    /// Flow-instance level tags that are applicable for completion of this specific execution / invocation of the flow (by invocationId).
    /// </summary>
    public KeyValuePair<string, object?>[]? FlowInstanceCompletionTags => _flowInstanceCompletionTags;
    ...
    {
        var tags = new KeyValuePair<string, object?>[flowTags.Length + 1];
        flowTags.CopyTo(tags, 0);
        tags[flowTags.Length] = new(DataFlowMetrics.TagNames.FlowInvocationId, invocationId);
        return tags;
    }
