...
/// These tests validate the solutions to the four main concerns:
...
/// </summary>
public class RevisedDiDesignTests
{
    ...
    [Fact]
    public void AddSingletonBlock_RegistersAsSingleton()
    {
        // Arrange
        var services = new ServiceCollection();
        services.AddDataFlows(df =>
        {
            df.AddSingletonBlock("test", sp => new TestProducerBlock());
        });
    ...
    [Fact]
    public void AddTransientBlock_RegistersAsTransient()
    {
        // Arrange
        var services = new ServiceCollection();
        services.AddDataFlows(df =>
        {
            df.AddTransientBlock("test", sp => new TestProducerBlock());
        });
    ...
    [Fact]
    public void SingletonBlocks_AreSameInstanceAcrossScopes()
    {
        // Arrange
        var services = new ServiceCollection();
        services.AddDataFlows(df =>
        ...
        Assert.NotNull(block1);
        Assert.NotNull(block2);
        Assert.Same(block1, block2); // Same instance across scopes
    }
    ...
    public void UnifiedRegistration_AllComponentsInOneCall()
    {
        ...
        services.AddDataFlows(df =>
        {
            ...
            df.AddSingletonBlock("processor", sp => new TestProcessorBlock());
    ...
    private class TestProducerBlock : IBlock
    {
    ...
    private class TestTransformerBlock : IBlock
    {
            ...
            IExecutionContext context)
        {
            await foreach (var item in input)
            {
                ...
                {
                    yield return intValue.ToString();
                }
            }
        }
    }

    private class TestProcessorBlock : IBlock
    {
