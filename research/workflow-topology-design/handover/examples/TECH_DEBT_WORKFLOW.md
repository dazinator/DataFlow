...
```bash
./research/workflow-topology-design/handover/prototype/query-workflow-queue.sh tech-debt
```

**Entry Points:**
- From Triage workflow (identified as tech debt)
- From Implementation workflow (debt discovered during implementation)
...
**When**: Tech debt analysis complete, all backlog items created

```bash
gh issue close $ISSUE --comment "✅ **Tech Debt Analysis Complete**

Analysis findings documented and backlog items created.

**Findings Report**: \`/research/tech-debt-[date]/README.md\`
**Backlog Items**: \`/product/backlog/techdebt-[date]-*.md\`
**Items Created**: [count] tech debt items

Product team will prioritize items for implementation."
```

### Handover to Product Prioritization

**When**: Analysis complete, items need prioritization before implementation
...
gh issue comment $ISSUE --body "📊 **Handover: Tech Debt → Product Prioritization**

Tech debt analysis complete. Backlog items need prioritization.
...
./research/workflow-topology-design/handover/prototype/handover-issue.sh \
  $ISSUE tech-debt product-backlog "Analysis complete, items need prioritization"
```
