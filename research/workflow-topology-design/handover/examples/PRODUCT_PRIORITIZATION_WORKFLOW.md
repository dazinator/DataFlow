...
```bash
./research/workflow-topology-design/handover/prototype/query-workflow-queue.sh product-backlog
```

**Optional: View with priority labels:**
```bash
gh issue list \
  --label "workflow:product-backlog" \
  --state open \
  --json number,title,labels \
  --jq '.[] | {number, title, priority: (.labels[] | select(.name | startswith("priority:")) | .name)}'
```

**Entry Points:**
- From Triage workflow (needs prioritization)
- From Research workflow (completed research, ready for prioritization)
- From Tech Debt workflow (backlog items created)
- Periodic backlog reviews
...
**When**: Items prioritized and ready for implementation
...
./research/workflow-topology-design/handover/prototype/handover-issue.sh \
  $ISSUE product-backlog implementation "Prioritized as high priority for implementation"
```

### Handover to Research

**When**: Item needs validation before implementation

```bash
./research/workflow-topology-design/handover/prototype/handover-issue.sh \
  $ISSUE product-backlog research "Needs validation before implementation"
```

### Close Issue

**When**: Item deprioritized and won't be implemented

```bash
gh issue close $ISSUE --comment "📊 **Deprioritized**

After prioritization review, this item will not be implemented.

**Reason**: [Business context for decision]"
```
