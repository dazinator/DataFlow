...
When research is complete, hand over to the appropriate next workflow:
...
```bash
./research/workflow-topology-design/handover/prototype/handover-issue.sh \
  $ISSUE research product-backlog "Research complete, needs prioritization"
```

### Handover to Triage

**When**: Research shows approach is not feasible

```bash
./research/workflow-topology-design/handover/prototype/handover-issue.sh \
  $ISSUE research triage "Approach not feasible, needs reassessment"
```

### Close Issue

**When**: Research shows solution is not viable
