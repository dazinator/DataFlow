...
| Date | Area | Improvement | Benefit | Scenario | PR |
|------|------|-------------|---------|----------|-----|
| 2025-11-13 | Triage, Automation, Issue Templates | Fixed scheduled triage workflow to comment on PR instead of issue; simplified process modeling template | Enables automated bulk triage via GitHub Actions and improves template accuracy (Scheduled workflow now triggers Copilot correctly by commenting on PR instead of issue; Graceful handling when no PR exists; Template references updated from old `.team/prompts/` to new `.team/duties/` structure; Simplified template reduces confusion by removing obsolete references) | 4 scenarios: baseline comment on issue (FAIL - expected, shows problem), improved comment on PR (PASS), edge case no PR (PASS - graceful), verify template refs (PASS - all correct) - 100% pass rate | TBD |
| 2025-11-12 | Triage, Kernel Layer | Added query_unlabeled_work_items semantic operation for complete triage coverage | Ensures 100% triage coverage by finding work items without workflow labels, preventing items from being missed (Added 13th semantic operation to kernel; Updated bulk triage procedure to query both labeled and unlabeled items; Discovered edge case where old procedure would miss ALL items; Zero kernel/dependency leaks; Complete backward compatibility) | 3 scenarios: bulk triage with unlabeled items (PASS), mixed labeled/unlabeled (PASS), edge case only unlabeled items (PASS - proves old procedure would fail) - 100% pass rate | TBD |
| 2025-11-11 | Process Modeling, Issue Templates | Workflow parameter extraction and issue template simplification guidance | Establishes systematic parameter management and reduces template maintenance burden by 15-30% (Decision framework for when to extract parameters vs keep inline; Template simplification eliminates procedural duplication; Single source of truth for workflow procedures; Improved maintainability and consistency) | 4 scenarios: baseline param identification (FAIL - no guidance), improved with extraction guidance (PASS), baseline template complexity (FAIL - duplication), improved simplified templates (PASS) | TBD |
| 2025-11-10 | Process Modeling | Simplified bulk process modeling template and fixed outdated rules | Reduces template bloat by 54% and improves accuracy (84→39 lines; fixed 400+ threshold as advisory not hard stop; removed duplicate instructions; workflow is single source of truth; user scan time reduced 67%) | 2 scenarios: baseline current (84 lines, outdated), improved simplified (39 lines, accurate) - PASS | TBD |
| 2025-11-10 | Research | Research workflow comprehensive guidance additions | Significantly improves research quality and reduces uncertainty by 40-50% (Added 4 major guidance sections: Approach Analysis patterns reduce documentation confusion; Prototyping Scope framework clarifies effort requirements; Benchmark Template ensures consistent performance validation; Hybrid Approach pattern supports complex design decisions) | Already-implemented check verified 2 of 6 suggestions present; 4 new comprehensive sections added based on actual user feedback | TBD |
| 2025-11-10 | Process Modeling | Separated workflow improvement template into single and bulk modes | Reduces cognitive load and friction by 75%, aligns with bulk triage pattern (Single template was overwhelming with 4 mode options; Two focused templates are clearer; Matches established bulk mode pattern for consistency) | 4 scenarios: single improvement (PASS), bulk processing (PASS), template comparison (PASS), empty queue edge case (PASS) | TBD |
| 2025-11-10 | All Workflows, Process Modeling | Workflow file ownership and supported label documentation | Prevents workflow files from being updated outside process modeling, eliminates label invention (Explicit ownership statement in 3 locations; Checklist prevents implementation touching workflow files; 6 supported labels clearly listed; No room for inventing new label formats; All workflow updates now get proper tabletop validation) | 4 scenarios: baseline implementation with workflow files (FAIL expected), improved implementation redirects to process modeling (PASS), workflow label validation (PASS), process modeling ownership clarity (PASS) | TBD |
